...
async function autoLoadRoomFromStorage() {
    const saved = localStorage.getItem("activeRoomCode");
    if (!saved) return false;
    try {
        // validate
        await api(`/rooms/${encodeURIComponent(saved)}`);
        setActiveRoom(saved);
        return true;
    } catch {
...
async function getMyPermanentRoom() {
    // Only for member/admin (endpoint is authorized by MemberOrAdmin)
    const r = await api("/member/room");
    // expected: { ok:true, roomId, roomCode, name }
    const code = (r?.roomCode || r?.roomId || "").trim();
    if (!code) throw new Error("/member/room returned no roomCode");
    return { code, raw: r };
}

async function isMemberOrAdmin() {
    const me = await refreshMe();
    return me?.isAuth && (me.role === "member" || me.role === "admin");
}
...
const GAME_CONTROL_SCHEMAS = {
    ...
    riddlemethis: {
        ...
        controls: [
            ...
            { key: "numRounds", label: "Number of rounds", type: "number", default: 5, min: 1, max: 20 }
        ]
    }
};
...
const GAMES_REGISTRY = {
    ...
    higherlower: {
        title: "Higher or Lower",
        playUrl: (code) => `/play.html?roomId=${encodeURIComponent(code)}&game=higherlower`,
        overlayUrl: (code) => `/overlay.html?roomId=${encodeURIComponent(code)}&game=higherlower`,
        start: async (code) => {
            const difficulty = String(gameSettings.higherlower?.difficulty ?? "easy").trim().toLowerCase();
            const okDiff = (difficulty === "easy" || difficulty === "medium" || difficulty === "hard");
            if (!okDiff) throw new Error("Higher or Lower: difficulty must be easy, medium, or hard");
            const nc = gameSettings.higherlower?.numCards;
            const numCards = Number.isFinite(nc) && nc > 0 ? nc : 5;
            return await api(`/rooms/${encodeURIComponent(code)}/games/higherlower/start`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ difficulty, numCards })
            });
        },
        stop: async (code) => api(`/rooms/${encodeURIComponent(code)}/games/stop`, { method: "POST" })
    },
    twentyone: {
        title: "Twenty One",
        playUrl: (code) => `/play.html?roomId=${encodeURIComponent(code)}&game=twentyone`,
        overlayUrl: (code) => `/overlay.html?roomId=${encodeURIComponent(code)}&game=twentyone`,
        start: async (code) => {
            const difficulty = String(gameSettings.twentyone?.difficulty ?? "easy").trim().toLowerCase();
            const okDiff = (difficulty === "easy" || difficulty === "medium" || difficulty === "hard");
            if (!okDiff) throw new Error("Twenty One: difficulty must be easy, medium, or hard");
            const nc = gameSettings.twentyone?.numCards;
            const numCards = Number.isFinite(nc) && nc > 0 ? nc : 5;
            return await api(`/rooms/${encodeURIComponent(code)}/games/twentyone/start`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ difficulty, numCards })
            });
    ...
    riddles: {
        title: "Riddle Me This",
        playUrl: (code) => `/play.html?roomId=${encodeURIComponent(code)}&game=riddlemethis`,
        overlayUrl: (code) => `/overlay.html?roomId=${encodeURIComponent(code)}&game=riddlemethis`,
        start: async (code) => {
            const difficulty = String(gameSettings.riddlemethis?.difficulty ?? "easy").trim().toLowerCase();
            const okDiff = (difficulty === "easy" || difficulty === "medium" || difficulty === "hard");
            if (!okDiff) throw new Error("Riddle Me This: difficulty must be easy, medium, or hard");
            const nc = gameSettings.riddlemethis?.numCards;
            const numCards = Number.isFinite(nc) && nc > 0 ? nc : 5;
            return await api(`/rooms/${encodeURIComponent(code)}/games/riddle_me_this/start`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ difficulty, numCards })
            });
...
async function init() {
    ...
    if (!me?.isAuth) {
        ...
        autoLoadRoomFromStorage();   // last room or none
        return;
    }
    ...
    if (me.role === "member" || me.role === "admin") {
        try {
            const mine = await getMyPermanentRoom();
            setActiveRoom(mine.roomCode);
        } catch (e) {
            console.warn("Permanent room unavailable:", e);
...
document.addEventListener("DOMContentLoaded", () => {
    init().catch(e => {
        console.error("Lobby init failed at DOMContent Level:", e);
        location.href = "/login.html";
    });
});